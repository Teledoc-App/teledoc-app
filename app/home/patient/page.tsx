			...
			<div className="bg-white w-screen max-w-[425px] h-fit flex flex-col justify-center items-center px-4 py-8 overflow-y-scroll">
				...
				<nav className="flex items-center w-full gap-4 mb-8">
					...
					<div>
						...
						<span className="text-black font-semibold text-[18px]">{userProfile?.name}</span>
					</div>
				</nav>
				...
				<section className="w-full">
					...
					<div>
						{filteredDoctors?.map((doctor, index) => (
							<a href={`../../doctors/appointment/${doctor.userId}`} key={index}>
								<div key={index} className="flex gap-4 w-full bg-[#d9d9d9]/30 rounded-lg p-6 mb-4">
									{/* PICTURE */}
									{doctor.user?.image == null ? (
										...
										<div className="w-[85px] h-[85px] rounded-full p-0 bg-white overflow-hidden">
											<img width={85} height={85} src={doctor.user?.image} alt="" />
										</div>
									)}

									{/* INFORMATIONS */}
									<div>
										<span className="text-[16px] font-semibold">{doctor?.username}</span>
										<p className="text-[12px] text-[#858585] mb-3">{doctor?.specialist.title}</p>
